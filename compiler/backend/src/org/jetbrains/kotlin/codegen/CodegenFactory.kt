...
object DefaultCodegenFactory : CodegenFactory {
    override fun generateModule(state: GenerationState, files: Collection<KtFile?>, errorHandler: CompilationErrorHandler) {
        ...
        for (file in files) {
            ...
            val fileClassInfo = state.fileClassesProvider.getFileClassInfo(file)
        ...
        for (multifileClassFqName in Sets.union(filesInMultifileClasses.keySet(), obsoleteMultifileClasses)) {
        ...
        for (packageFqName in Sets.union(packagesWithObsoleteParts, filesInPackages.keySet())) {
            CodegenFactory.doCheckCancelled(state)
