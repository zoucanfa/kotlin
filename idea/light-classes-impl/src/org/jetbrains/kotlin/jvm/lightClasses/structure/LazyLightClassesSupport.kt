...
import org.jetbrains.kotlin.jvm.lightClasses.structure.builder.LightClassBuilderResult
...
interface LazyLightClassesSupport {
    fun createDataHolderForClass(classOrObject: KtClassOrObject, builder: LightClassBuilder): LightClassDataHolder.ForClass

    fun createDataHolderForFacade(files: Collection<KtFile>, builder: LightClassBuilder): LightClassDataHolder.ForFacade

    fun resolveToDescriptor(declaration: KtDeclaration): DeclarationDescriptor?

    fun analyze(element: KtElement): BindingContext

    fun analyzeFully(element: KtElement): BindingContext

    companion object {
        fun getInstance(project: Project) = ServiceManager.getService(project, LazyLightClassesSupport::class.java)
    }
}